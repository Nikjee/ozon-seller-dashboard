...
export function useTransactionTotals() {
  ...
  async function load() {
    ...
    try {
      ...
      raw.value = await invoke('get_finance_detailed', { dateFrom, dateTo })
    } catch (e) {
  ...
  const data = computed(() => raw.value?.totals || null)
  const expenses = computed(() => raw.value?.expenses || null)
  ...
  return {
    month, year, raw, loading, error,
    data, expenses,
    accrualsForSale, totalCompensation, saleCommission,
